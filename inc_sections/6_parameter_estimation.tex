...
\begin{equation}
\overline{elaP}_{(c,cty)} = \alpha^{pe}_{c} + \beta^{pe}_{c} \cdot \log(\overline{gdpPc}_{cty})
\label{eq:pe_reg}
\end{equation}
...
1 - \overline{sysShr}_{(a\_liv,bas,cty,cz,sys)}
}
\right)
=
\delta^{sys}_{(a\_liv,cz,sys)} + \rho^{sys}_{(a\_liv,sys)} \cdot \log(\overline{gdpPc}_{cty})
\label{eq:sys_reg}
\end{equation}
...
\begin{align}
\log \left( \overline{feedReq}_{(a\_liv,bas,cty,cz,sys)} \right)
&=
\alpha^{fe}_{(a\_liv,cz,sys)} + \beta^{fe}_{(a\_liv,cz,sys)} \cdot \log(\overline{gdpPc}_{cty}) \label{eq:fe_reg}\\[6pt]
\log \left(
\frac{
\overline{feedConcShr}_{(a\_liv,bas,cty,cz,sys)}
}{
1 - \overline{feedConcShr}_{(a\_liv,bas,cty,cz,sys)}
}
\right)
&=
\delta^{fs}_{(a\_liv,cz,sys)} + \rho^{fs}_{(a\_liv,sys)} \cdot \log(\overline{gdpPc}_{cty})
\label{eq:fs_reg}
\end{align}
...
1 - 
\frac{
\overline{RRed}_{(src,gas,cty)}
}{
\overline{mrp}_{(src,gas,cty)}
}
\right)
=
\sigma_{(src,gas,cty)} \cdot \log(\overline{pCar}_{cty} + 1)
\label{eq:sigma_reg}
\end{equation}
...
The parameters $\delta^{sys}_{(a\_liv,cz,sys)}$ and $\rho^{sys}_{(a\_liv,sys)}$
...
\subsubsection{Relationship between feed requirements and per-capita GDP}

Figure~\ref{fig:feed_req_gdp} shows the relationship between per-capita GDP and feed requirements
by livestock type, climate zone, and production system.
Table~\ref{tab:feed_req_gdp} presents the estimated regression parameters,
...
\begin{figure}[H]
    ...
    \caption{Relationship between per-capita GDP and feed requirements}
    \label{fig:feed_req_gdp}
\end{figure}
...
\caption{Estimated parameters of feed requirements (95\% confidence intervals in parentheses)}
\label{tab:feed_req_gdp}
\resizebox{\textwidth}{!}{%
\csvreader[
