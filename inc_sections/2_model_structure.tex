...
focusing on agricultural markets and land use, operating with an annual time step. The model uses 2015 as the base year 
and projects up to 2100. It is formulated as a nonlinear complementarity problem (NCP) and handles approximately 200,000 variables. 

On the spatial dimension, the demand side covers 166 countries and regions globally, while the production side consists of 
400 production units. These production units are defined by overlaying the 166 demand regions with FAO’s 230 major river basin boundaries. 
Figure~\ref{fig:regions} illustrates the geographical coverage of the demand regions and production units.

The basic model structure is shown in Figure~\ref{fig:model_structure}. Food production is categorized into 13 primary crops, 
6 livestock products, and 4 processed goods. Table~\ref{tab:commodities} lists the food commodity classifications used in the model. 
...
land inputs, are modeled at the level of the 166 demand regions. Land use within each production unit is allocated according to a hierarchical 
...
(imports minus exports) is maintained. In the global market, total world imports and exports are also balanced. Furthermore, commodities 
that were not produced or traded in the base year are assumed to remain inactive in that country or production unit throughout the projection period.


\begin{figure}[tbp]
  \centering
  \includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{fig1}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{fig2}
  \caption{Geographical coverage of demand regions (166) and production units (400).}
  \label{fig:regions}
\end{figure}
