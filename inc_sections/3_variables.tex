...
\begin{xltabular}{\linewidth}{l X l X}
  ...
  \texttt{lu\_prot} & Protected land-use class & 5 & \texttt{primf}, \texttt{secdf}, \texttt{gra}, \texttt{pastr}, \texttt{range} \\
  \texttt{lu\_pas} & Protected pasture class & 2 & \texttt{pastr}, \texttt{range} \\
...
\begin{xltabular}{\linewidth}{l X l}
  ...
  \texttt{PLGRA} & Other natural land price & \$/ha \\
  ...
  \texttt{PLAFRCOMP} & Complementary variable for the annual afforestation limit & - \\
  \texttt{PLPRIMFPROTCOMP} & Complementary variable for protected primary forest & - \\
  \texttt{PLSECDFPROTCOMP} & Complementary variable for protected secondary forest & - \\
  \texttt{PLGRAPROTCOMP} & Complementary variable for protected other natural land & - \\
  \texttt{PLPASPROTCOMP} & Complementary variable for protected pasture & - \\
  \texttt{PLLIMCOMP} & Complementary variable for the cropland constraint & - \\
\end{xltabular}
...
\begin{xltabular}{\linewidth}{l X l}
  ...
  \texttt{QST} & Stock change plus the base-year statistical calibration adjustment & FAOSTAT FBS \\
  ...
  \texttt{YLDBASE} & Crop yield in the base year & FAOSTAT, GAEZ~v4; H08 for energy crops \\
  \texttt{YLD} & Annual crop yield & Model calculation \\
  \texttt{YLDTECH} & Yield change factor due to technological progress & IMPACT \\
  \texttt{YLDCHAN} & Yield change factor due to climate change impacts & GAEZ~v4 \\
  ...
  \texttt{SYSSHARE} & Annual livestock system share & Model calculation \\
  \texttt{ICL} & Annual feed input coefficient & Model calculation \\
  ...
  \texttt{PLPRIMFBASE} & Primary forest land price in the base year & LUH2, GTAP~10 \\
  \texttt{PLSECDFBASE} & Secondary forest land price in the base year & LUH2, GTAP~10 \\
  \texttt{PLGRA\_C} & Price of other natural land before the protection constraint & LUH2, GTAP~10 \\
  \texttt{PLAFR\_C} & Afforested land price before the annual planting constraint & AIM-PLUM, VISIT \\
  \texttt{PLCROFAL} & Fallow land price & LUH2, GTAP~10 \\
  \texttt{CARBONSTOCK} & Forest carbon stock by production unit & AIM-PLUM, VISIT \\
  \texttt{CARBONSTOCK\_GRA} & Carbon stock of other natural land by production unit & AIM-PLUM, VISIT \\
...
\begin{xltabular}{\linewidth}{l X X}
  ...
  \texttt{qlafr\_age} & Afforested area by age class & Records annual afforestation by stand age.\\
